	...
	if($arquivo_som["name"] != ''){
		...
		if (in_array( $ext_som, $array_ext_sons )){
			if ($arquivo_som['size'] < 1024 * 1024 * 8){ //8 megas, para alterar este valor, tem que alterar tambem no php.ini
				...
				echo "<script > alert('Som enviado!'); </script>";
			}
			else{
				echo '<script> alert("Som com um tamanho muito grande"); </script>';
			}
		}
		else{
			echo '<script> alert("O formato do som escolhido esta incorreto."); </script>';
		}
	}
	else{
