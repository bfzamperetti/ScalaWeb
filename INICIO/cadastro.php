...
<div class="logo_scala_capa">
	...
	<iframe src="../INCLUDES/termos_de_uso_espanhol.html" style="margin-top: 5px; border: 1px dashed #115511;" width="100%" height="155px;"></iframe>
	<?php } else {?>
	<iframe src="../INCLUDES/termos_de_uso_ingles.html" style="margin-top: 5px; border: 1px dashed #115511;" width="100%" height="155px;"></iframe>
	<?php } ?>
			...
			<form name="frm_cadastro" action="cadastrar.php" method="post" onsubmit="return cadastrar();">
				...
				<table>
					...
					<tr>
						...
						<td>
							<select onclick="if (typeof(this.selectedIndex) != 'undefined') if (this.selectedIndex == 5) $('#profissao_outros').show(); else $('#profissao_outros').hide();" class="inputs" name="profissao" id="profissao">
					...
					<tr id="profissao_outros" style="display: none;">
						<td align="right">
							<label for="profissao_outro"><?php echo $_str['lblOther']; ?>:</label>
						</td>
						<td >
							<input type="text" name="profissao_outros" class="inputs" />
						</td>
					</tr>
					<tr>
...
<script type="text/javascript">
	var login_cad = $( "#login_cad" ),
		...
		allFields = $( [] ).add( login_cad ).add( senha_cad ).add( confirm_senha ).add( nome ).add( cidade ).add( email ).add( termosdeuso ),
		tips = $( ".validateTips" );
