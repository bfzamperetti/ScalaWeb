	...
	if($arquivo_img["name"] != ''){
		...
		if (in_array( $ext_img, $array_ext_imgs )){
			if ($arquivo_img['size'] < 1024 * 1024 * 8){ //8 megas, para alterar este valor, tem que alterar tambem no php.ini
				...
				$qry = pg_query($sql);
				echo "<script >  escolherCatImagensHist(8, 0); jAlert('".$_str['lblImageSent']."!','".$_str['lblImageWarning']."'); </script>";
			}
			else{
				echo '<script> jAlert("'.$_str['lblImageHasLargeSize'].'.","'.$_str['lblImageWarning'].'"); </script>';
			}
		}
		else{
			echo '<script> jAlert("'.$_str['lblImageHasWrongFileFormat'].'.","'.$_str['lblImageWarning'].'"); </script>';
		}
	}
	else{
		echo '<script> jAlert("'.$_str['lblNoFileWasSent'].'!","'.$_str['lblImageWarning'].'"); </script>';
	}
}
