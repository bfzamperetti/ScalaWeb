<?php
	...
	if ($_GET['cat'] == 0){ //se for uma requisição de "MINHAS IMAGENS"
		...
		while ($imgs = pg_fetch_array($qry, NULL, PGSQL_ASSOC)){
			...
			echo "<div class='qdr_imagem_lista' id='qdr_imagem_lista_varredura".$imgs['id']."' onclick='javascript: selecionar(".$imgs['id'].", 1);'>
		...
		while ($imgs = pg_fetch_array($qry, NULL, PGSQL_ASSOC)){
			...
			echo "<div class='img_imagem_lista_varredura' id='qdr_imagem_lista".$imgs['id']."' onclick='javascript: selecionar(".$imgs['id'].", 1);'>";
