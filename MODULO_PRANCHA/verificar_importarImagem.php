	...
	if($arquivo_img["name"] != ''){
		...
		if (in_array( $ext_img, $array_ext_imgs )){
			if ($arquivo_img['size'] < 1024 * 1024 * 8){ //8 megas, para alterar este valor, tem que alterar tambem no php.ini
				...
				$qry = pg_query($sql);
				echo "<script>  escolherCatImagens(8, 0); jAlert('Imagem enviada!','Aviso de Imagem'); </script>";
			}
			else{
				echo '<script> jAlert("A imagem tem um tamanho muito grande.","Aviso de Imagem"); </script>';
			}
		}
		else{
			echo '<script> jAlert("O formato da imagem escolhido esta incorreto.","Aviso de Imagem"); </script>';
		}
	}
	else{
		echo '<script> jAlert("Nenhum arquivo foi enviado!","Aviso de Imagem"); </script>';
	}
}
